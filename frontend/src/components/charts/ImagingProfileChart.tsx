...
const ImagingProfileChart: React.FC<ImagingProfileChartProps> = ({ data }) => {
  ...
  const datasets = allPossibleLabels
    .map(label => ({
      label,
      data: [
        Number((((data.raiox.find(d => d.label === label)?.count || 0) / raioxTotal) * 100).toFixed(1)),
        Number((((data.tomo.find(d => d.label === label)?.count || 0) / tomoTotal) * 100).toFixed(1)),
      ],
  ...
  const { canvasRef } = useChartJs(
    () => ({
      ...
      options: {
        ...
        scales: {
          x: { stacked: true, min: 0, max: 100, ticks: { callback: (v: string | number) => `${v}%` } },
          y: { stacked: true },
        },
      },
    }),
