...
const BoxPlot: React.FC<BoxPlotProps> = ({
  ...
  useEffect(() => {
    ...
    g.append('g')
      ...
      .call((gg) =>
        gg.selectAll('.tick line').attr('stroke', themeColors.grid).attr('stroke-dasharray', '3,3'),
      )
      .call((gg) =>
        gg
          .selectAll('text')
      ...
      .on('mouseenter', (e) => {
        onHover(
          ...
          <div>
            ...
            <br />
            Q1: {stats.q1}d<br />
            Mediana: <strong>{stats.median}d</strong>
            <br />
            Q3: {stats.q3}d<br />
        ...
        {criterios.map((c, i) => (
          <button
            ...
            style={{
              ...
              gap: '6px',
              fontSize: '11px',
              color: hidden.has(c.label) ? themeColors.neutral : themeColors.text,
              background: 'none',
              border: 'none',
              padding: 0,
              cursor: 'pointer',
...
const AntiviralKPIs: React.FC<{ items: AntiviralItem[]; themeColors: Record<string, string> }> = ({
  items,
  themeColors,
}) => {
  ...
  const getBadgeColors = (status: string) => {
    ...
    if (status === 'ok')
      return isDark ? { bg: '#134e4a', color: '#5eead4' } : { bg: '#e1f5ee', color: '#085041' };
    if (status === 'warn')
      return isDark ? { bg: '#451a03', color: '#fbbf24' } : { bg: '#faeeda', color: '#633806' };
          ...
          <div style={{ position: 'relative', width: '120px', height: '120px' }}>
            <svg width="120" height="120" viewBox="0 0 120 120">
              ...
              <circle
                ...
                r="54"
                fill="none"
                stroke={data.coberturaTestagem.rate >= 80 ? themeColors.ok : themeColors.warn}
                strokeWidth="12"
                strokeDasharray={`${(data.coberturaTestagem.rate / 100) * 339.29} 339.29`}
                strokeLinecap="round"
            ...
            >
              <span style={{ fontSize: '24px', fontWeight: 800, color: themeColors.main }}>
                {data.coberturaTestagem.rate}%
              </span>
            </div>
          </div>
          <p style={{ marginTop: '1rem', fontSize: '12px', color: themeColors.text }}>
            <strong>{data.coberturaTestagem.collected}</strong> amostras de{' '}
            <strong>{data.coberturaTestagem.total}</strong> casos
          </p>
        </div>
      ),
    },
      ...
      content: (
        ...
        >
          ...
          {data.distribuicaoAmostra.length === 0 && (
            <p style={{ textAlign: 'center', fontSize: '12px', color: themeColors.neutral }}>
