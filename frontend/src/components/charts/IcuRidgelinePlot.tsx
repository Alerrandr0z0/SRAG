...
const IcuRidgelinePlot: React.FC<IcuRidgelinePlotProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const statsByKey = d3.rollup(
        ...
        (v) => {
          ...
          const waitData = v
            ...
            .sort(d3.ascending);
          const medianWait =
            waitData.length > 0 ? (d3.median(waitData) ?? 0) : 0;
          const p75 =
            waitData.length > 0 ? (d3.quantile(waitData, 0.75) ?? 0) : 0;
          const p90 =
            waitData.length > 0 ? (d3.quantile(waitData, 0.9) ?? 0) : 0;
          return {
            total,
      ...
      const bandwidth =
        0.9 * globalStddev * Math.pow(allWaits.length, -0.2);
      const bw = Math.max(bandwidth, 0.25);

      const labelMap = Object.fromEntries(
        filteredAll.map((d) => [d.groupKey, d.displayLabel]),
      );
      ...
      const containerWidth =
        svgRef.current.parentElement?.clientWidth || 820;
      const margin = { top: 36, right: 24, bottom: 44, left: 152 };
      const width = containerWidth - margin.left - margin.right;
      const rowHeight = 52;
      const ridgeHeightMax = rowHeight * 0.82;
      const height =
        groups.length * rowHeight + margin.top + margin.bottom;

      svg
        .attr("width", containerWidth)
        .attr("height", height)
        .attr("viewBox", `0 0 ${containerWidth} ${height}`)
        .style("font-family", "'IBM Plex Sans', 'Inter', sans-serif");
      ...
      const getGradId = (median: number) => {
        if (median <= WARN_THRESHOLD) return "url(#fill-good)";
        if (median <= CRIT_THRESHOLD) return "url(#fill-warn)";
        return "url(#fill-crit)";
      };
      ...
      ridgeGroups
        ...
        .attr("fill", (d) => getGradId(d.median))
        .attr("stroke", "none");
      ...
      // Median dot (on the curve)
      ridgeGroups
        .append("circle")
        .attr("cx", (d) => x(d.median))
        .attr("cy", (d) => {
          const nearestPt = d.normDensity.reduce((prev, cur) =>
            Math.abs(cur.x - d.median) < Math.abs(prev.x - d.median)
              ? cur
              : prev,
          );
    ...
    <div style={{ width: "100%", position: "relative" }}>
      ...
      >
        {[
          {
            color: "#0f766e",
            label: `Adequado (mediana ≤ ${WARN_THRESHOLD}d)`,
          },
          {
            color: "#d97706",
            label: `Atenção (${WARN_THRESHOLD}–${CRIT_THRESHOLD}d)`,
          },
          {
            color: "#dc2626",
            label: `Crítico (mediana > ${CRIT_THRESHOLD}d)`,
          },
        ].map(({ color, label }) => (
          <div
            key={label}
      ...
      >
        Linha sólida vertical = mediana · tracejada = P75 · rótulo =
        % admitidos no mesmo dia (verde ≥85%, laranja ≥70%, vermelho &lt;70%)
      </div>
          ...
          >
            ...
            >
              {[
                ...
                {
                  label: "Aguardaram",
                  value: String(tooltip.waitingCases),
                },
                {
                  label: "Admissão no mesmo dia",
                  value: `${tooltip.sameDayRate.toFixed(1)}%`,
                },
                {
                ...
                { label: "P75 espera", value: `${tooltip.p75.toFixed(1)}d` },
                { label: "P90 espera", value: `${tooltip.p90.toFixed(1)}d` },
            ...
            <div
              style={{
                ...
                borderRadius: "4px",
                background:
                  tooltip.median <= WARN_THRESHOLD
                    ? "#f0fdf4"
                    : tooltip.median <= CRIT_THRESHOLD
                    ? "#fffbeb"
                    : "#fef2f2",
                textAlign: "center",
                fontSize: "10px",
