...
def main() -> None:
    print("Iniciando motor universal de ingestão...")
    ...
    if not files:
        print("⚠️ Nenhum dado encontrado em data/raw/.")
        return
        ...
        try:
            ...
            for col in target_cols:
                ...
                if col == "unique_hash":
                    ...
                    orig = get_col(col_up)
                    select_parts.append(
                        f"COALESCE(TRY_CAST({orig} AS DATE), TRY_CAST(strptime(CAST({orig} AS VARCHAR), '%d/%m/%Y') AS DATE))"
                    )
                else:
