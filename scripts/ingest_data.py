...
def _build_projection_select(
    ...
    def resolve_hash_field(field: str) -> str:
        if field == "ID_MUNICIP":
            return source_mun
        return file_cols.get(field.upper(), "NULL")
    ...
    for col in target_cols:
        ...
        if col == "unique_hash":
            ...
            orig = get_col("CO-DETEC") if get_col("CO-DETEC") != "NULL" else get_col("CO_DETEC")
            select_parts.append(orig)
        elif col_up == "FAB_COV1":
            orig = get_col("FAB_COV_1") if get_col("FAB_COV_1") != "NULL" else get_col("FAB_COV1")
            select_parts.append(orig)
        elif col_up == "FAB_COV2":
            orig = get_col("FAB_COV_2") if get_col("FAB_COV_2") != "NULL" else get_col("FAB_COV2")
            select_parts.append(orig)
