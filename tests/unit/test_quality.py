    ...
    df = pd.DataFrame(
        {
            ...
            "DT_PCR": [
                ...
                "2023-03-01",
                "2023-01-05",
            ],  # 1st is negative delta, 2nd is > 30, 3rd is valid
        }
    )
    res = compute_diagnostic_latency(df)
    assert res["count"] == 1
...
def test_sample_type_distribution_valid() -> None:
    df = pd.DataFrame({"TP_AMOSTRA": [1, 1, 2, "3", 9, 10, "invalid"]})
    res = compute_sample_type_distribution(df)
...
def test_testing_coverage_valid() -> None:
    df = pd.DataFrame({"AMOSTRA": [1, "1", 2, 9, "invalid", None]})
    res = compute_testing_coverage(df)
    assert res["total"] == 6
...
def test_data_completeness_valid() -> None:
    ...
    def assert_rate(group_name: str, field_name: str, expected: float) -> None:
        group = next(g for g in res if g["group"] == group_name)
        field = next(f for f in group["fields"] if f["field"] == field_name)
        assert field["rate"] == expected, (
            f"Failed {group_name} -> {field_name}: got {field['rate']} expected {expected}"
        )
