...
def test_classificar_status_gripe() -> None:
    ...
    # Ignorado
    row_ign = pd.Series({"NU_IDADE_N": 30, "TP_IDADE": 3, "VACINA": 9, "DT_SIN_PRI": "2023-05-01"})
    ...
    df = pd.DataFrame(
        {
            ...
            "DT_NOTIFIC": [
                ...
                "2023-01-10",
                "2023-01-01",
            ],  # third is delay=-2 (should be ignored)
        }
    )
...
def test_compute_antiviral_usage_valid() -> None:
    df = pd.DataFrame({"CLASSI_FIN": [1, 1, 1, 5], "ANTIVIRAL": [1, 2, 9, 1]})
...
def test_compute_time_series_valid() -> None:
    df = pd.DataFrame({"DT_SIN_PRI": ["2023-01-01", "2023-01-02"]})
...
def test_compute_genomic_variants_valid() -> None:
    df = pd.DataFrame(
        {"VG_OMS": [1, 2, 1], "DT_SIN_PRI": ["2023-01-01", "2023-01-01", "2023-01-15"]}
    )
...
def test_compute_lethality_heatmap_empty() -> None:
    assert compute_lethality_heatmap(pd.DataFrame()) == {
        "agents": [],
        "age_bands": [],
        "matrix": [],
    }
...
def test_compute_codetection_matrix_valid() -> None:
    df = pd.DataFrame({"CO_DETEC": [1, 1, 2], "PCR_SARS2": [1, 1, 1], "PCR_VSR": [1, 0, 0]})
...
def test_compute_imaging_profile_valid() -> None:
    df = pd.DataFrame({"RAIOX_RES": [1, 2, 9], "TOMO_RES": [1, 3, 9]})
...
def test_compute_serology_profile_valid() -> None:
    df = pd.DataFrame({"TP_SOR": [1, 2], "RES_IGG": [1, 2], "RES_IGM": [3, 1]})
...
def test_compute_antiviral_types_valid() -> None:
    df = pd.DataFrame({"TP_ANTIVIR": [1, 2], "TIPO_TRAT": [1, 2]})
...
def test_compute_mortality_by_treatment_agent_valid() -> None:
    df = pd.DataFrame({"CLASSI_FIN": [1, 5, 4], "SUPORT_VEN": [1, 2, 3], "EVOLUCAO": [2, 2, 1]})
