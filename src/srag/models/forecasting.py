...
def predict_next_weeks(
    ...
    # Aplicamos um log-transform ou cap para estabilizar o modelo se os valores forem muito discrepantes.
    ...
    try:
        ...
        for _, row in forecast_future.iterrows():
            ...
            pred = int(round(max(0, float(row['yhat']))))
            lower = int(round(max(0, float(row['yhat_lower']))))
            upper = int(round(max(0, float(row['yhat_upper']))))
