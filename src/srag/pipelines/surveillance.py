...
def _setup_duckdb_and_schema(db_path: Path) -> tuple[duckdb.DuckDBPyConnection, list[str]]:
    """Configure DuckDB connection, attach DB, and ensure schema exists."""
    con = duckdb.connect()
    ...
    return con, target_cols


def _find_data_files(data_dirs: list[Path]) -> list[Path]:
...
def _build_surv_projection_select(
    target_cols: list[str],
    file_map: dict[str, str],
    s_mun: str,
    s_res: str,
) -> list[str]:
    """Build select expressions for each target column in the surveillance pipeline."""

    def get_src(name: str) -> str:
        return file_map.get(name.upper(), "NULL")
...
def _ingest_source(
    ...
    """Read a data file, map target columns, and insert matched records into temp_raw."""
    for source in _load_surveillance_sources(con, pf):
        ...
        select_parts = _build_surv_projection_select(target_cols, file_map, s_mun, s_res)
...
def _load_and_deduplicate(con: duckdb.DuckDBPyConnection, target_cols: list[str]) -> None:
    """Remove older/duplicated cases and insert distinct cases into sqlite_db."""
...
def _run_geographical_intelligence(db_path: Path) -> None:
    """Normalize neighborhoods (BAIRRO_REF) and infer zones (ZONA) in cases_srag table."""
    with sqlite3.connect(db_path) as conn:
...
def _generate_surveillance_snapshot(
    con: duckdb.DuckDBPyConnection,
    db_path: Path,
    start_time: datetime,
) -> tuple[int, Path]:
    """Export week metadata metrics snapshot to snap JSON."""
...
def run_surveillance_pipeline(
    ...
    try:
        ...
        con, target_cols = _setup_duckdb_and_schema(db_path)

        files = _find_data_files(data_dirs)

        for pf in files:
            _ingest_source(con, pf, target_cols)
        ...
        # 3. Load & Deduplicate
        _load_and_deduplicate(con, target_cols)

        # 4. Intelligence Pass (Bairros/Zonas)
        _run_geographical_intelligence(db_path)

        # 5. Snapshot generation
        final_count, snap_file = _generate_surveillance_snapshot(con, db_path, start_time)
